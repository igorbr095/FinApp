...
const diasTrabalhados = (cfg) => { const d=new Date(); const [y,m]=cfg.mes.split("-").map(Number); if(y!==d.getFullYear()||m!==d.getMonth()+1) return cfg.dias||16; return Math.max(1,Math.round((cfg.dias||16)*(d.getDate()/new Date(y,m,0).getDate()))); };

// ── SUPABASE CONFIG ───────────────────────────────────────────────────────────
const SB_URL = "https://znvdplshycsmojvjicdu.supabase.co";
const SB_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InpudmRwbHNoeWNzbW9qdmppY2R1Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzkwMjAxMjgsImV4cCI6MjA5NDU5NjEyOH0.U_YtRbDS5AA6_LjfWjxHrjid8FVd-W7eTXbodGlt8sw";
...
// ── STORAGE por usuário ───────────────────────────────────────────────────────
async function sGet(key, token, userId) {
  try {
    const res = await fetch(`${SB_URL}/rest/v1/fin_dados?user_id=eq.${userId}&chave=eq.${encodeURIComponent(key)}&select=valor`, {
      headers:{"apikey":SB_KEY,"Authorization":`Bearer ${token}`}
    });
    const data = await res.json();
    console.log("📦 sGet:", key, "→", data?.length, "registros");
...
async function sSet(key, val, token, userId) {
  try {
    const res = await fetch(`${SB_URL}/rest/v1/fin_dados`, {
      method:"POST",
      headers:{"apikey":SB_KEY,"Authorization":`Bearer ${token}`,"Content-Type":"application/json","Prefer":"resolution=merge-duplicates"},
      body:JSON.stringify({user_id:userId,chave:key,valor:val,updated_at:new Date().toISOString()})
    });
    const txt = await res.text();
    if(!res.ok) {
      console.error("❌ sSet erro:", res.status, txt);
      // fallback localStorage
      try { localStorage.setItem(key, val); } catch(e2) {}
    } else {
      console.log("✅ sSet ok:", key);
    }
  } catch(e) {
...
export default function App() {
  ...
  // refs sempre atualizados — evita closure stale nos saves
  ...
  useEffect(()=>{
    (async()=>{
      ...
      const savedToken = localStorage.getItem("fin_token");
      const savedUserId = localStorage.getItem("fin_userid");
      const savedEmail = localStorage.getItem("fin_email");
      if(savedToken && savedUserId) {
        // verificar se token ainda é válido
        const user = await authGetUser(savedToken);
        if(user?.id) {
          setToken(savedToken); setUserId(savedUserId); setUserEmail(savedEmail||"");
          await loadUserData(savedToken, savedUserId);
          return;
        } else {
          localStorage.removeItem("fin_token");
          localStorage.removeItem("fin_userid");
  ...
  async function loadUserData(tk, uid) {
    let cfg={dias:16,mes:mesAtual()};
    try{const r=await sGet("fin4_config",tk,uid);if(r)cfg={...cfg,...JSON.parse(r)};}catch(e){}
    setConfig(cfg); setCfgDias(String(cfg.dias)); setCfgMes(cfg.mes); setFPpInicio(cfg.mes);
    try{const r=await sGet("fin4_rf_"+cfg.mes,tk,uid);if(r)setRecFixas(JSON.parse(r));}catch(e){}
    try{const r=await sGet("fin4_fl_"+cfg.mes,tk,uid);if(r)setRendas(JSON.parse(r));}catch(e){}
    try{const r=await sGet("fin4_pes",tk,uid);if(r)setPessoais(JSON.parse(r));}catch(e){}
    try{const r=await sGet("fin4_fix_"+cfg.mes,tk,uid);if(r)setFixos(JSON.parse(r));}catch(e){}
    try{const r=await sGet("fin4_hist",tk,uid);if(r)setHistorico(JSON.parse(r));}catch(e){}
    setReady(true);
  }

  async function handleLogin() {
    setAuthErr(""); setAuthLoad(true);
    const data = await authSignIn(authEmail, authPass);
    if(data?.access_token) {
      localStorage.setItem("fin_token", data.access_token);
      localStorage.setItem("fin_userid", data.user.id);
      localStorage.setItem("fin_email", data.user.email);
      setToken(data.access_token); setUserId(data.user.id); setUserEmail(data.user.email);
      await loadUserData(data.access_token, data.user.id);
    } else {
      setAuthErr(data?.error_description || data?.msg || "E-mail ou senha incorretos.");
    }
    setAuthLoad(false);
  }
  ...
  async function handleSignUp() {
    setAuthErr(""); setAuthLoad(true);
    const data = await authSignUp(authEmail, authPass);
    if(data?.user?.id || data?.access_token) {
      if(data?.access_token) {
        ...
        setAuthErr("Conta criada! Faça login agora.");
        setAuthPage("login");
      }
    } else {
      setAuthErr(data?.error_description || data?.msg || "Erro ao criar conta. Tente outro e-mail.");
    }
    setAuthLoad(false);
  }
  ...
  async function handleLogout() {
    ...
    localStorage.removeItem("fin_userid");
    localStorage.removeItem("fin_email");
    setToken(null); setUserId(null); setUserEmail("");
    setMenuOpen(false);
    // resetar dados
    setRecFixas([]); setRendas([]); setPessoais([]); setFixos([]); setHistorico({});
    setConfig({dias:16,mes:mesAtual()});
  }

  // atualizar refs sempre que mudar
  useEffect(()=>{ tokenRef.current=token; },[token]);
  useEffect(()=>{ userIdRef.current=userId; },[userId]);
  useEffect(()=>{ mesRef.current=config.mes; },[config.mes]);

  const saveRf  = async(d)=>await sSet("fin4_rf_"+mesRef.current,JSON.stringify(d),tokenRef.current,userIdRef.current);
  const saveFl  = async(d)=>await sSet("fin4_fl_"+mesRef.current,JSON.stringify(d),tokenRef.current,userIdRef.current);
  const savePes = async(d)=>await sSet("fin4_pes",JSON.stringify(d),tokenRef.current,userIdRef.current);
  const saveFix = async(d)=>await sSet("fin4_fix_"+mesRef.current,JSON.stringify(d),tokenRef.current,userIdRef.current);
  const saveHist= async(d)=>await sSet("fin4_hist",JSON.stringify(d),tokenRef.current,userIdRef.current);
  ...
  const dt             = diasTrabalhados(config);
  const rendaNec       = Math.max(0, totalSaidas - totalRecFixas - totalLiqFlex + totalBrutoFlex > 0 ? totalSaidas - totalRecFixas : totalSaidas - totalRec + totalBrutoFlex);
  const uberLiqAtual   = rendasComLiq.filter(r=>r.tipo==="uber").reduce((s,r)=>s+r.liquido,0);
  const metaUberLiq    = Math.max(0, totalSaidas - totalRecFixas);
  const faltaUber      = Math.max(0, metaUberLiq - uberLiqAtual);
  const diariaNec      = dr>0 ? faltaUber/dr : 0;
  const parcsAtivas    = pessoais.filter(p=>parcelaMes(p,config.mes)>0).length;

  // ── navegação entre meses ───────────────────────────────────────────────────
  async function navegarMes(direcao) {
    const novoMes = mesOffset(config.mes, direcao);
    const cfg2 = {...config, mes:novoMes};
    setConfig(cfg2); setCfgMes(novoMes);
    await sSet("fin4_config", JSON.stringify(cfg2), token, userId);
    try{const r=await sGet("fin4_rf_"+novoMes,token,userId);setRecFixas(r?JSON.parse(r):[]);}catch(e){setRecFixas([]);}
    try{const r=await sGet("fin4_fl_"+novoMes,token,userId);setRendas(r?JSON.parse(r):[]);}catch(e){setRendas([]);}
    try{const r=await sGet("fin4_fix_"+novoMes,token,userId);setFixos(r?JSON.parse(r):[]);}catch(e){setFixos([]);}
  }

  const atualizarHist = async(nRf,nFl,nFix,nPes)=>{
    ...
    const gas=(nFix||fixos).reduce((s,x)=>s+x.valor,0)+(nPes||pessoais).reduce((s,p)=>s+parcelaMes(p,mesRef.current),0);
    const h={...historico,[mesRef.current]:{rec,gas}};
    ...
    <div style={{background:C.bg,minHeight:"100vh",fontFamily:"'DM Sans',sans-serif",color:C.text,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      ...
      <div style={{width:"100%",maxWidth:380}}>
        <div style={{textAlign:"center",marginBottom:32}}>
          ...
          <div style={{fontSize:14,color:C.text2}}>Gestão financeira pessoal</div>
        </div>
        <div style={{background:C.bg2,border:`0.5px solid ${C.border}`,borderRadius:16,padding:24}}>
          <div style={{display:"flex",marginBottom:24,background:C.bg3,borderRadius:10,padding:3}}>
            {["login","signup"].map(p=>(
              <button key={p} onClick={()=>{setAuthPage(p);setAuthErr("");}} style={{flex:1,height:36,border:"none",borderRadius:8,fontSize:14,fontWeight:600,cursor:"pointer",fontFamily:"inherit",background:authPage===p?C.green:"transparent",color:authPage===p?C.bg:C.text2,boxShadow:authPage===p?glow(C.green,6):"none",transition:"all 0.2s"}}>
                {p==="login"?"Entrar":"Criar conta"}
              </button>))}
          </div>
          <div style={{marginBottom:14}}>
            <label style={{fontSize:12,color:C.text2,marginBottom:6,display:"block"}}>E-mail</label>
            <input style={{...inp}} type="email" placeholder="seu@email.com" value={authEmail} onChange={e=>setAuthEmail(e.target.value)} onKeyDown={e=>e.key==="Enter"&&(authPage==="login"?handleLogin():handleSignUp())}/>
          </div>
          <div style={{marginBottom:20}}>
            <label style={{fontSize:12,color:C.text2,marginBottom:6,display:"block"}}>Senha</label>
            <input style={{...inp}} type="password" placeholder="mínimo 6 caracteres" value={authPass} onChange={e=>setAuthPass(e.target.value)} onKeyDown={e=>e.key==="Enter"&&(authPage==="login"?handleLogin():handleSignUp())}/>
          </div>
          {authErr&&<div style={{fontSize:13,color:authErr.includes("Conta criada")?C.green:C.red,marginBottom:14,textAlign:"center",lineHeight:1.5}}>{authErr}</div>}
          <button onClick={authPage==="login"?handleLogin:handleSignUp} disabled={authLoad||!authEmail||!authPass} style={{...btn(authLoad?C.bg3:C.green,C.bg,C.green),opacity:(authLoad||!authEmail||!authPass)?0.5:1}}>
            {authLoad?"⏳ Aguarde...":(authPage==="login"?"Entrar":"Criar conta")}
          </button>
        </div>
        <div style={{textAlign:"center",marginTop:16,fontSize:12,color:C.text3}}>Seus dados ficam salvos na nuvem com segurança</div>
      </div>
    </div>
  );
    ...
    <div style={{background:C.bg,minHeight:"100vh",fontFamily:"'DM Sans',sans-serif",color:C.text}}>
      ...
      {menuOpen&&<div style={{position:"fixed",inset:0,zIndex:200,display:"flex"}}>
        ...
        <div style={{position:"relative",width:270,background:C.bg2,borderRight:`0.5px solid ${C.border}`,height:"100%",display:"flex",flexDirection:"column",zIndex:1,paddingTop:52}}>
          ...
          <div style={{padding:"12px 20px 32px",borderTop:`0.5px solid ${C.border}`}}>
            <div style={{fontSize:11,color:C.text3,marginBottom:8}}>Logado como</div>
            <div style={{fontSize:12,color:C.text2,marginBottom:12,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{userEmail}</div>
            <button onClick={handleLogout} style={{width:"100%",height:36,background:"transparent",color:C.red,border:`0.5px solid ${C.red}40`,borderRadius:8,fontSize:13,fontWeight:500,cursor:"pointer",fontFamily:"inherit"}}>Sair da conta</button>
          </div>
        </div>
      </div>}
      ...
      <div style={{position:"sticky",top:0,zIndex:100,background:`${C.bg}f0`,borderBottom:`0.5px solid ${C.border}`,padding:"14px 20px",display:"flex",alignItems:"center",gap:14,backdropFilter:"blur(20px)"}}>
        ...
        <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:6}}>
          <button onClick={()=>navegarMes(-1)} style={{background:"none",border:`0.5px solid ${C.border}`,borderRadius:8,width:28,height:28,color:C.text2,cursor:"pointer",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center"}}>‹</button>
          <div style={{fontSize:11,color:C.text2,background:C.bg3,border:`0.5px solid ${C.border}`,borderRadius:20,padding:"4px 11px",whiteSpace:"nowrap"}}>{fmtMesLong(config.mes)}</div>
          <button onClick={()=>navegarMes(1)} style={{background:"none",border:`0.5px solid ${C.border}`,borderRadius:8,width:28,height:28,color:C.text2,cursor:"pointer",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center"}}>›</button>
        </div>
      </div>
      ...
      <div style={{padding:20,paddingBottom:40}}>
        ...
        {page==="dashboard"&&<>
          ...
          {/* meta uber */}
          {metaUberLiq>0&&<div style={{...card,marginBottom:14,background:`${C.amber}08`,border:`0.5px solid ${C.amber}30`,boxShadow:glow(C.amber,4)}}>
            <div style={{...clbl,color:C.amber}}>🚗 Meta Uber do mês</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:12}}>
              <div><div style={{fontSize:10,color:C.text3,marginBottom:4}}>Necessário</div><div style={{fontSize:14,fontWeight:700,fontFamily:"'DM Mono',monospace",color:C.amber,textShadow:glow(C.amber,4)}}>{fmtBRLShort(metaUberLiq)}</div></div>
              <div><div style={{fontSize:10,color:C.text3,marginBottom:4}}>Gerado</div><div style={{fontSize:14,fontWeight:700,fontFamily:"'DM Mono',monospace",color:C.green,textShadow:glow(C.green,4)}}>{fmtBRLShort(uberLiqAtual)}</div></div>
              <div><div style={{fontSize:10,color:C.text3,marginBottom:4}}>Falta</div><div style={{fontSize:14,fontWeight:700,fontFamily:"'DM Mono',monospace",color:faltaUber<=0?C.green:C.red,textShadow:glow(faltaUber<=0?C.green:C.red,4)}}>{faltaUber<=0?"✓ OK":fmtBRLShort(faltaUber)}</div></div>
            </div>
            <ProgBar pct={metaUberLiq>0?(uberLiqAtual/metaUberLiq*100):100} color={faltaUber<=0?C.green:C.amber}/>
            {faltaUber>0&&dr>0&&<div style={{fontSize:12,color:C.text2,marginTop:6}}>Precisa de <strong style={{color:C.amber}}>{fmtBRL(diariaNec)}/dia</strong> líquido nos próximos {dr} dia(s)</div>}
            {faltaUber<=0&&<div style={{fontSize:12,color:C.green,marginTop:6}}>🎉 Meta Uber atingida este mês!</div>}
          </div>}
